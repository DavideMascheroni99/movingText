...
# Function to trim CSV files: first 1500 rows
...
# Prepare file paths
expected = []
for tester in range(1, fv_constant.number_of_testers + 1):
...
if os.path.exists(fv_constant.FIX_VECTOR_PATH_TEN):
    os.remove(fv_constant.FIX_VECTOR_PATH_TEN)
pd.DataFrame(fpogs_vector).to_csv(fv_constant.FIX_VECTOR_PATH_TEN, index=False)
...
f11_rows = []
for d in fpogs_vector:
    distances = [distance.euclidean([d['FPOGX'][i], d['FPOGY'][i]], [d['FPOGX'][i+1], d['FPOGY'][i+1]])
                 for i in range(len(d['FPOGX']) - 1)]
    row = {'file_key': d['file_key']}
    for i, func in enumerate(stat_funcs):
        row[f'f{11+i}'] = func(distances) if distances else 0
    f11_rows.append(row)
...
for d in fpogs_vector:
    speeds = []
    for i in range(len(d['FPOGX']) - 1):
        dist = distance.euclidean([d['FPOGX'][i], d['FPOGY'][i]], [d['FPOGX'][i+1], d['FPOGY'][i+1]])
        time = d['FPOGS'][i+1] - d['FPOGS'][i] - d['FPOGD'][i]
        if time > 0:
            speeds.append(dist / time)
    row = {'file_key': d['file_key']}
    for i, func in enumerate(stat_funcs):
        row[f'f{21+i}'] = func(speeds) if speeds else 0
    spd_rows.append(row)
...
f31_rows = []
for d in fpogs_vector:
    total = np.sum([distance.euclidean([d['FPOGX'][i], d['FPOGY'][i]], [d['FPOGX'][i+1], d['FPOGY'][i+1]])
                    for i in range(len(d['FPOGX']) - 1)])
...
for key, df in all_dfs.items():
    ...
    row = {'file_key': key}
    for i, func in enumerate(stat_funcs):
        row[f'f{32+i}'] = func(lpd) if lpd else 0
    lpd_rows.append(row)
...
for key, df in all_dfs.items():
    ...
    row = {'file_key': key}
    for i, func in enumerate(stat_funcs):
        row[f'f{42+i}'] = func(rpd) if rpd else 0
    rpd_rows.append(row)
...
for key, df in all_dfs.items():
    ...
    row = {'file_key': key}
    for i, func in enumerate(stat_funcs):
        row[f'f{52+i}'] = func(ratio) if ratio else 0
...
for key, df in all_dfs.items():
    ...
    for i, func in enumerate(diff_funcs):
        row[f'f{62+i}'] = func(diff) if diff else 0
...
for key, df in all_dfs.items():
    ...
    row['f73'] = np.min(bkdur) if bkdur else 0
    row['f74'] = np.max(bkdur) if bkdur else 0
    blink_rows.append(row)
features.append(pd.DataFrame(blink_rows))

# f75–f82: directional movements
direction_rows = []
for d in fpogs_vector:
    file_key = d['file_key']
    ...
    for i in range(len(d['FPOGX']) - 1):
        ...
        if dy < 0: dx_up += abs(dy); cnt_up += 1
        elif dy > 0: dx_down += abs(dy); cnt_down += 1
        if dx > 0: dx_right += abs(dx); cnt_right += 1
        elif dx < 0: dx_left += abs(dx); cnt_left += 1
    row = {'file_key': file_key, 'f75': dx_up, 'f76': dx_down, 'f77': dx_left, 'f78': dx_right,
...
# Save final CSV
if os.path.exists(fv_constant.TEN_OUT_PATH):
    os.remove(fv_constant.TEN_OUT_PATH)

final.to_csv(fv_constant.TEN_OUT_PATH, index=False)
print("Saved:", fv_constant.TEN_OUT_PATH)
