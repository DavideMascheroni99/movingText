...
The total number of blinks in a trial (\(f71\)) was determined by counting the distinct blink identifiers (\(\text{BKID}\)), restricted to values greater than zero to exclude missing or invalid entries. 
This feature provides a measure of blink frequency, which can vary substantially across individuals and contexts.

In addition, the temporal properties of blinks were captured using the duration values (\(\text{BKDUR}\)), measured in milliseconds. 
