...
# Prepare file paths
expected = []
for tester in range(1, fv_constant.number_of_testers_static + 1):
    for session in range(1, 4):
        for trial in range(1, 4):
            for anim in fv_constant.animnames_static:  
                path = os.path.join(
                    fv_constant.BASE_PATH_STATIC,
                    f"Tester{tester}", f"Session{session}", f"Trial{trial}",
                    f"T{tester}-S{session}-TRY{trial}-{anim}.csv"
                )
                expected.append((path, f"T{tester}_S{session}_TRY{trial}_{anim}"))
...
if os.path.exists(fv_constant.FIX_VECTOR_PATH_EIGHT_S):
    os.remove(fv_constant.FIX_VECTOR_PATH_EIGHT_S)

# Save fixation vector
pd.DataFrame(fpogs_vector).to_csv(fv_constant.FIX_VECTOR_PATH_EIGHT_S, index=False)
...
if os.path.exists(fv_constant.EIGHT_S_OUT_PATH):
    os.remove(fv_constant.EIGHT_S_OUT_PATH)

final.to_csv(fv_constant.EIGHT_S_OUT_PATH, index=False)
print("Saved:", fv_constant.EIGHT_S_OUT_PATH)
